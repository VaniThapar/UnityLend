...
-- Create borrow_request_community_map table
...
-- Create borrow_request_EMI table
CREATE TABLE borrow_request_EMI (
    emi_id CHAR(36) PRIMARY KEY,
    ...
    FOREIGN KEY (borrow_request_id) references borrow_request(borrow_request_id),
    FOREIGN KEY (emi_status) references status(status_code)
);
